...
class marabouEncoding:
    ...
    def checkFair(self):
        ...
        sen_pre_11.addAddend(1, self.inputVars_verified[0][0])
        sen_pre_11.setScalar(0)
        sen_pre_12 = MarabouCore.Equation(MarabouCore.Equation.EQ)
        sen_pre_12.addAddend(1, self.inputVars_verified[0][1])
        ...
        sen_pre_21.addAddend(1, self.inputVars_verified[1][0])
        sen_pre_21.setScalar(1)
        sen_pre_22 = MarabouCore.Equation(MarabouCore.Equation.EQ)
        sen_pre_22.addAddend(1, self.inputVars_verified[1][1])
    ...
    def checkFairRegion(self, region):
        ...
        for i in range(len(self.inputVars_verified)):
            for j in range(len(self.inputVars_verified[0])):
                if j == 2:  # quantitative
                    ...
                    self.network_verified.setLowerBound(
                        self.inputVars_verified[i][j], region[5][0]
                    )
                    self.network_verified.setUpperBound(
                        self.inputVars_verified[i][j], region[5][1]
                    )
                elif j == 15:
                    self.network_verified.setLowerBound(
                        self.inputVars_verified[i][j], region[8][0]
                    )
                    self.network_verified.setUpperBound(
                        self.inputVars_verified[i][j], region[8][1]
                    )
                elif j == 21:
                    self.network_verified.setLowerBound(
                        self.inputVars_verified[i][j], region[11][0]
                    )
                    self.network_verified.setUpperBound(
                        self.inputVars_verified[i][j], region[11][1]
                    )
                elif j == 22:
                    self.network_verified.setLowerBound(
                        self.inputVars_verified[i][j], region[12][0]
                    )
                    self.network_verified.setUpperBound(
                        self.inputVars_verified[i][j], region[12][1]
                    )
                else:
                    self.network_verified.setLowerBound(
        ...
        ####################
        for i in range(len(self.inputVars_verified)):
            for j, sub_region in zip(
                [0, 2, 3, 4, 7, 10, 11, 13, 15, 16, 18, 21, 22], region
            ):
                if j in [4, 7, 18]:  # Three variable
                ...
                elif j in [11, 13, 16]:  # Two variable
                    if not (-1 in sub_region):
                        # One-hot encoding constraint
                        disjunction_onehot_two = []
                        eq_pre_01 = MarabouCore.Equation(MarabouCore.Equation.EQ)
