...
class FairNet(nn.Module):
    def __init__(self):
        ...
        self.fc4_prop = nn.Linear(node_num, node_num)
...
def parse_arguments():
    ...
    text = "Japanese Credit Screening dataset PyTorch Training DNN"
...
def main():
    ...
    # read from dataset
    data = pd.read_csv(
        "../../TrainingData/japanese/japanese-processed.csv",
        header=None,
        sep=",",
    )
    ...
    transform_dict = {}
    for index in row:
        if index in [1, 2, 5, 8, 11, 12]:  # normalization for quantitative value
            Scaler = MinMaxScaler(feature_range=(0, 1))
            col_value = np.array(data[index]).reshape(-1, 1)
            new_col = Scaler.fit_transform(col_value)
            data[index] = new_col
        elif index == 13:  # label from str to int
            transform_dict["+"] = [1]
            transform_dict["-"] = [0]
        else:
    ...
    data = data.applymap(lambda x: transform_dict[x] if type(x) != float else x)
    data = np.array(data)
    ...
    for row in final_data:
        ...
        for i in row:
            if type(i) == int or type(i) == float:
                ...
                flatList.extend(i)
        final_data_list.append(flatList)

    final_label_list = []
    for row in final_label:
        flatList = []
        for i in row:
            flatList.append(i)
        final_label_list.append(flatList)

    print(len(final_data_list[0]))
    dataset = Data.TensorDataset(
        torch.tensor(final_data_list), torch.tensor(final_label_list)
    )
    ...
    if args.save_model:
        ...
        for name, param in model.named_parameters():
            if name == "fc_input_prop.weight":
                ...
                dict_params_copy[name].data.copy_(arr)
            elif name == "fc4_prop.weight":
                arr = dict_params_copy["fc4.weight"].data
                dict_params_copy[name].data.copy_(arr)
            elif name == "fc4_prop.bias":
                arr = dict_params_copy["fc4.bias"].data
                dict_params_copy[name].data.copy_(arr)
        ...
        prefix_folder = "../../networks/japanese/"
        if args.rqfolder != "nonrq":
            prefix_folder += args.rqfolder + "/"
        torch.save(
        ...
        for name, param in model_half.named_parameters():
            if name == "fc_input.weight":
                ...
                arr = dict_params_ori["fc4.weight"].data
                dict_params_half[name].data.copy_(arr)
            elif name == "fc4.bias":
                arr = dict_params_ori["fc4.bias"].data
                dict_params_half[name].data.copy_(arr)
