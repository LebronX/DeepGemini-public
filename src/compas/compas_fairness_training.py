...
def parse_arguments():
    ...
    text = "Compas dataset PyTorch Training DNN"
...
def main():
    ...
    # read from dataset
    data = pd.read_csv(
        "../../TrainingData/compas/compas-" + str(args.datasettype) + "-processed.csv",
        header=None,
        sep=",",
    )
    ...
    transform_dict = {}
    for index in row:
        if index in [3, 4]:  # normalization for quantitative value
            Scaler = MinMaxScaler(feature_range=(0, 1))
            col_value = np.array(data[index]).reshape(-1, 1)
            new_col = Scaler.fit_transform(col_value)
            data[index] = new_col
        elif index == 8:  # label from str to int
            transform_dict["High"] = [0]
            transform_dict["Medium"] = [1]
            transform_dict["Low"] = [1]
        else:
