...
from compas_fairness_training import FairHalfNet
...
def random_split_region(cur_region, input_dimension):
    ...
    while (
        (split_dimension in [1] and cur_region[split_dimension].count(-1) == 2)
        or (
            split_dimension in [0, 5, 6, 7]
            and cur_region[split_dimension].count(-1) == 1
        )
        or split_dimension == 2
...
def compute_region_percentage(cur_region, total_fair_region_set):
    ...
    """
    total_space_size = 480000  # 3*2*2*2*2*100*100
    dimension_3_helper_list = [
        i for i in range(int(cur_region[3][0] * 100), int(cur_region[3][1] * 100))
    ]
    dimension_4_helper_list = [
        i for i in range(int(cur_region[4][0] * 100), int(cur_region[4][1] * 100))
    ]
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=1,
        metavar=1,
        help="Query 1: Is there bias with respect to race?",
    )
...
def main():
    ...
    init_region = [
        [0, 1],  # gender, 2, [0-1]
        [0, 1, 2],  # age, 3, [2-4]
        [0, 1, 2, 3, 4, 5],  # XXX race, 6, [5-10]
        [0, 1],  # quantitative, 1, 11
        [0, 1],  # quantitative, 1, 12
        [0, 1],  # binary, 2, [13-14]
        [0, 1],  # binary, 2, [15-16]
        [0, 1],  # binary, 2, [17-18]
    ]
    ...
    if prop[0] == "checking-fairness":
        ...
        while time_total < args.time:
            ...
            if time_total > record_second and args.record == "True":
                write_line_csv(csvfile)
                record_second += 1
            iter += 1
